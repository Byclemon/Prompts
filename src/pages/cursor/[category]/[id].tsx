...
export default function CursorDetail({ prompt }: PromptDetailProps) {
...
export const getStaticPaths: GetStaticPaths = async ({ locales = [] }) => {
  try {
    // 获取所有提示词列表
    const prompts = await getPromptList('cursor')
...
export const getStaticProps: GetStaticProps = async ({ params, locale }) => {
  ...
  try {
    const promptPath = `cursor/${params.category}/${params.id}`
